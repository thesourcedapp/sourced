...
const THEMES: Record<ThemeKey, Theme> = {
  ...
  white:  { name:'WHITE',  desc:'Clean pure white',         bg:'#ffffff', surface:'#f8f8f8', border:'#e8e8e8', text:'#000',   muted:'#aaa',   accent:'#000',    accentText:'#fff', cardBg:'#f8f8f8' },
          ...
          <div className="max-w-6xl mx-auto">
              ...
              <div className="flex-1 min-w-0 space-y-3 md:space-y-4 pt-1">
                ...
                <div>
                  ...
                  <div className="flex flex-wrap items-center gap-4">
                    <h1 className="text-5xl md:text-7xl lg:text-8xl font-black tracking-tighter leading-[0.9]" style={{fontFamily:'Archivo Black, sans-serif',color:T.text}}>@{profile.username}</h1>
                ...
                {profile.contact_email&&(
                  <div className="flex flex-wrap gap-2">
                    <a href={`mailto:${profile.contact_email}`} style={{fontFamily:'Bebas Neue, sans-serif',borderColor:T.border,color:T.muted,textDecoration:'none'}} className="flex items-center gap-1.5 px-2.5 py-1 text-[9px] tracking-[0.2em] font-black border transition-all hover:opacity-100 opacity-70"
                      onMouseEnter={e=>{(e.currentTarget as HTMLAnchorElement).style.borderColor=T.text;(e.currentTarget as HTMLAnchorElement).style.color=T.text;}}
                      onMouseLeave={e=>{(e.currentTarget as HTMLAnchorElement).style.borderColor=T.border;(e.currentTarget as HTMLAnchorElement).style.color=T.muted;}}>
              ...
              <div className="mt-8 p-5 border" style={{borderColor:T.border,backgroundColor:T.surface}}>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
                  {[
                    ...
                    {label:'UNIQUE CLICKS',value:analytics?analytics.unique_clicks.toLocaleString():'—'},
            ...
            {topBrands.length > 0 && (
              <div className="mt-6">
                ...
                <div className="flex flex-wrap gap-2">
                  {topBrands.filter(b=>!hiddenBrands.has(b.seller)).map((b,i)=>(
                    <div key={i} className="flex items-center gap-2 px-3 py-1.5 border group/brand relative" style={{borderColor:T.border,backgroundColor:T.cardBg}}>
                      ...
                      {isOwner&&<button onClick={()=>setHiddenBrands(prev=>{const n=new Set(prev);n.add(b.seller);return n;})} className="ml-1 opacity-0 group-hover/brand:opacity-100 transition-opacity text-[9px]" style={{color:T.muted,background:'none',border:'none',cursor:'pointer'}} title="Hide this brand">✕</button>}
                    </div>
                  ))}
                  {isOwner&&hiddenBrands.size>0&&(
                    <button onClick={()=>setHiddenBrands(new Set())} className="px-3 py-1.5 text-[8px] tracking-wider font-black border border-dashed transition-all hover:opacity-100 opacity-50" style={{fontFamily:'Bebas Neue, sans-serif',borderColor:T.border,color:T.muted,backgroundColor:'transparent'}}>
                      SHOW ALL ({hiddenBrands.size} hidden)
                    </button>
                  )}
                </div>
              </div>
...
{/* no create in tab row - handled below */}
            </div>
          </div>
        </div>

        {/* CREATE BUTTONS — prominent row, owner only */}
        {isOwner&&(
          <div className="px-5 md:px-10 py-4" style={{borderBottomColor:T.border,borderBottomWidth:1,borderBottomStyle:'solid',backgroundColor:T.surface}}>
            <div className="max-w-6xl mx-auto flex items-center gap-3">
              <p className="text-[9px] tracking-[0.35em] font-black mr-2" style={{fontFamily:'Bebas Neue, sans-serif',color:T.muted}}>CREATE</p>
              <button onClick={()=>router.push('/app/catalogs')} className="flex items-center gap-2 px-5 py-2.5 text-[10px] tracking-[0.25em] font-black border-2 transition-all hover:opacity-90" style={{fontFamily:'Bebas Neue, sans-serif',borderColor:T.accent,backgroundColor:T.accent,color:T.accentText}}>
                <svg width="11" height="11" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path d="M12 5v14M5 12h14"/></svg>
                NEW CATALOG
              </button>
              <button onClick={()=>router.push('/posts/new')} className="flex items-center gap-2 px-5 py-2.5 text-[10px] tracking-[0.25em] font-black border-2 transition-all hover:opacity-90" style={{fontFamily:'Bebas Neue, sans-serif',borderColor:T.border,color:T.text,backgroundColor:'transparent'}}>
                <svg width="11" height="11" fill="none" stroke="currentColor" strokeWidth="2.5" viewBox="0 0 24 24"><path d="M12 5v14M5 12h14"/></svg>
                NEW POST
              </button>
            </div>
        ...
        <div className="px-5 md:px-10 py-8">
          <div className="max-w-6xl mx-auto">
            ...
            {activeTab==='catalogs'&&(catalogs.length===0
              ? <EmptyState text="NO CATALOGS YET" theme={T} cta={isOwner?'+ CREATE CATALOG':undefined} onCta={isOwner?()=>router.push('/app/catalogs'):undefined}/>
            ...
            <div className="p-5 space-y-5">
              ...
              {editTab==='profile'&&(
                <form onSubmit={saveProfile} className="space-y-5">
                  ...
                  <div className="space-y-2">
                    <label className="block text-[9px] tracking-[0.35em] font-black" style={{fontFamily:'Bebas Neue, sans-serif',color:T.muted}}>CONTACT EMAIL <span className="opacity-50">(shown publicly)</span></label>
              ...
              {editTab==='collab'&&(
                <div className="space-y-5">
                  ...
                  <button onClick={async()=>{if(!currentUserId)return;setSaving(true);try{await supabase.from('profiles').update({collab_status:editCollabStatus,collab_types:editCollabTypes.trim()||null}).eq('id',currentUserId);await loadProfile();}catch(e){console.error('collab save - run migrations first:',e);}finally{setSaving(false);}}} disabled={saving} className="w-full py-3.5 text-[10px] tracking-[0.4em] font-black border-2 transition-all disabled:opacity-40" style={{fontFamily:'Bebas Neue, sans-serif',borderColor:T.accent,backgroundColor:T.accent,color:T.accentText}}>{saving?'SAVING...':'SAVE STATUS'}</button>

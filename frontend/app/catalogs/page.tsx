  ...
  return(
    <>
      <style jsx global>{`
        ...
        .social-press{transition:transform 0.18s ease, box-shadow 0.18s ease;}
        .social-press:hover{transform:scale(1.05);}
        .social-press:active{transform:scale(0.95);}
      `}</style>
      ...
      <div style={{backgroundColor:T.bg,color:T.text,minHeight:'100vh'}}>
              ...
              <div className="flex flex-col md:flex-row gap-0">
                ...
                <div className="flex-1 md:pl-8 lg:pl-12 pt-6 md:pt-0 flex flex-col justify-between" style={{minHeight:'0'}}>
                  <div>
                    ...
                    {/* Social links — highlighted, clearly pressable */}
                    {(profile.social_instagram||profile.social_tiktok||profile.social_url||profile.contact_email)&&(
                      <div className="flex flex-wrap gap-2.5 mb-6">
                        {profile.social_instagram&&(
                          <a href={`https://instagram.com/${profile.social_instagram.replace('@','')}`} target="_blank" rel="noopener noreferrer" className="social-press" style={{fontFamily:'Bebas Neue, sans-serif',border:'none',color:'#fff',textDecoration:'none',background:'linear-gradient(45deg,#F58529 0%,#DD2A7B 50%,#8134AF 100%)',padding:'9px 16px',display:'flex',alignItems:'center',gap:'8px',fontSize:'10px',letterSpacing:'0.22em',fontWeight:'900',cursor:'pointer',boxShadow:'0 2px 10px rgba(225,48,108,0.35)'}}>
                        ...
                        {profile.social_tiktok&&(
                          <a href={`https://tiktok.com/@${profile.social_tiktok.replace('@','')}`} target="_blank" rel="noopener noreferrer" className="social-press" style={{fontFamily:'Bebas Neue, sans-serif',border:'none',color:'#fff',textDecoration:'none',backgroundColor:'#000',padding:'9px 16px',display:'flex',alignItems:'center',gap:'8px',fontSize:'10px',letterSpacing:'0.22em',fontWeight:'900',cursor:'pointer',boxShadow:'2px 2px 0 #ff0050, -2px -2px 0 #00f2ea'}}>
                        ...
                        {profile.social_url&&(
                          <a href={profile.social_url.startsWith('http')?profile.social_url:`https://${profile.social_url}`} target="_blank" rel="noopener noreferrer" className="social-press" style={{fontFamily:'Bebas Neue, sans-serif',border:`2px solid ${T.accent}`,color:T.text,textDecoration:'none',backgroundColor:`${T.accent}22`,padding:'9px 16px',display:'flex',alignItems:'center',gap:'8px',fontSize:'10px',letterSpacing:'0.22em',fontWeight:'900',cursor:'pointer'}}>
                            <svg width="12" height="12" fill="none" stroke="currentColor" strokeWidth="2.2" viewBox="0 0 24 24"><path d="M10 13a5 5 0 007.54.54l3-3a5 5 0 00-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 00-7.54-.54l-3 3a5 5 0 007.07 7.07l1.71-1.71"/></svg>
                            {profile.social_url.replace(/^https?:\/\//,'').split('/')[0]}
                          </a>
                        )}
                        {profile.contact_email&&(
                          <a href={`mailto:${profile.contact_email}`} className="social-press" style={{fontFamily:'Bebas Neue, sans-serif',backgroundColor:T.accent,color:T.accentText,textDecoration:'none',border:'none',padding:'9px 18px',display:'flex',alignItems:'center',gap:'8px',fontSize:'10px',letterSpacing:'0.25em',fontWeight:'900',cursor:'pointer',boxShadow:`0 2px 10px ${T.accent}55`}}>
                            <svg width="12" height="12" fill="none" stroke="currentColor" strokeWidth="2.2" viewBox="0 0 24 24"><path d="M4 4h16c1.1 0 2 .9 2 2v12c0 1.1-.9 2-2 2H4c-1.1 0-2-.9-2-2V6c0-1.1.9-2 2-2z"/><polyline points="22,6 12,13 2,6"/></svg>
                            CONTACT ME
                          </a>
                        )}
                      </div>
        ...
        <div className="px-5 md:px-10 py-7">
          <div className="max-w-6xl mx-auto">
            ...
            {activeTab==='catalogs'&&(catalogs.length===0
              ?<div className="text-center py-20"><p className="text-2xl font-black opacity-20" style={{fontFamily:'Bebas Neue, sans-serif',color:T.text}}>NO CATALOGS YET</p>{isOwner&&<button onClick={()=>router.push('/catalogs')} className="mt-4 px-6 py-3 text-[10px] tracking-[0.4em] font-black border-2" style={{fontFamily:'Bebas Neue, sans-serif',borderColor:T.accent,color:T.accent,backgroundColor:'transparent'}}>+ CREATE CATALOG</button>}</div>
              :<div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 md:gap-4">
        ...
        {/* ══════════════════════════
            ...
            Owner: navigate (analytics/catalogs/posts) + analytics stats + liked + saved
            Visitor: basic links
        ══════════════════════════ */}
        <div className="fixed inset-0 z-[60] pointer-events-none" style={{visibility:showMenu?'visible':'hidden'}}>
          ...
          <div className={`menu-in${showMenu?' open':''} absolute right-0 top-0 bottom-0 w-72 overflow-y-auto`} style={{backgroundColor:T.bg,borderLeft:`1px solid ${T.border}`,pointerEvents:'auto'}}>
            ...
            <div className="px-5 space-y-1 pb-8">
              {isOwner&&(
                <>
                  {/* NAVIGATE section — analytics, catalogs, posts */}
                  <p className="text-[8px] tracking-[0.5em] pt-4 pb-2 font-black" style={{fontFamily:'Bebas Neue, sans-serif',color:T.muted}}>NAVIGATE</p>
                  <button onClick={()=>{setShowMenu(false);router.push('/creator/analytics');}} className="w-full flex items-center gap-3 px-4 py-3 text-left transition-all hover:opacity-70" style={{border:`1px solid ${T.border}`,backgroundColor:T.cardBg}}>
                    <svg width="14" height="14" fill="none" stroke={T.accent} strokeWidth="2" viewBox="0 0 24 24"><path d="M3 3v18h18"/><path d="M7 14l3-3 4 4 5-5"/></svg>
                    <span className="text-[11px] font-black tracking-wider" style={{fontFamily:'Bebas Neue, sans-serif',color:T.text}}>ANALYTICS</span>
                  </button>
                  <button onClick={()=>{setShowMenu(false);router.push('/catalogs');}} className="w-full flex items-center gap-3 px-4 py-3 text-left transition-all hover:opacity-70" style={{border:`1px solid ${T.border}`,backgroundColor:T.cardBg}}>
                    <svg width="14" height="14" fill="none" stroke={T.accent} strokeWidth="2" viewBox="0 0 24 24"><rect x="3" y="3" width="7" height="7"/><rect x="14" y="3" width="7" height="7"/><rect x="3" y="14" width="7" height="7"/><rect x="14" y="14" width="7" height="7"/></svg>
                    <span className="text-[11px] font-black tracking-wider" style={{fontFamily:'Bebas Neue, sans-serif',color:T.text}}>CATALOGS</span>
                  </button>
                  ...
                  <p className="text-[8px] tracking-[0.5em] pt-5 pb-2 font-black" style={{fontFamily:'Bebas Neue, sans-serif',color:T.muted}}>QUICK STATS</p>
              ...
              {editTab==='profile'&&(
                <form onSubmit={saveProfile} className="space-y-5">
                  ...
                  <div className="space-y-3">
                    ...
                    <label htmlFor="avatar-upload" className="block w-full py-3 text-center text-[10px] tracking-[0.4em] font-black border-2 transition-all cursor-pointer hover:opacity-80" style={{fontFamily:'Bebas Neue, sans-serif',borderColor:T.border,color:T.text,backgroundColor:T.cardBg}}>CHANGE PHOTO</label>
                    <input id="avatar-upload" type="file" accept="image/*" onChange={handleAvatarFileSelect} className="hidden"/>
